...
export const SYSTEMS = [
  ...
  },
  {
    ...
    auth: XKEY,
    // This API strips names, emails, phones, attachments and free-text notes
    // before they leave the server, and lists what it withheld per row — so its
    // per-candidate endpoint carries no PII and stays available.
    redactsPeopleServerSide: true,
    probe: { path: 'summary', params: { days: 7 } },
    summary: {
      path: 'summary',
      params: (w) => (w === 'mtd' ? fromTo(w) : { days: Number(w.replace('d', '')) }),
